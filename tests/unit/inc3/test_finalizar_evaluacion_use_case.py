...
async def _evaluacion_en_curso(event_store: FakeEventStore):
...
class TestFinalizarEvaluacionUseCase:
    async def test_finaliza_una_evaluacion_en_curso(self):
    ...
    async def test_finaliza_una_evaluacion_suspendida(self):
        event_store = FakeEventStore()
        evaluacion_id, estudiante_id = await _evaluacion_en_curso(event_store)
        await event_store.append(
            AGGREGATE_TYPE_EVALUACION,
    ...
    async def test_rechaza_evaluacion_ya_finalizada(self):
        event_store = FakeEventStore()
        evaluacion_id, estudiante_id = await _evaluacion_en_curso(event_store)
        use_case = FinalizarEvaluacionUseCase(event_store)
