...
class TestEvaluacionesControllerFinalizar:
    async def _iniciar(self):
        materia_id, estudiante_id = uuid4(), uuid4()
        actividad_id = uuid4()
        apertura = datetime.now(UTC) - timedelta(days=1)
    ...
    async def test_finalizar_delega_al_use_case(self):
        controller, evaluacion, estudiante_id = await self._iniciar()

        finalizada = await controller.finalizar_evaluacion(evaluacion.id, estudiante_id)

        assert finalizada.estado == EstadoEvaluacion.FINALIZADA
