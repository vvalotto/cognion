"""Steps BDD de US-5.1.3 (`tests/features/inc5/US-5.1.3-notificacion-cierre-actividad.feature`)."""
...
class _FakeSmtpServer:
    ...
    Mismo motivo que `_FakeSmtpServer` de `tests/step_defs/inc5/test_us_5_1_2_steps.py`: debe
    ...
    def mensajes_de_cierre(self) -> list[str]:
        """Filtra los mensajes de cierre — la creación de la actividad ya dispara uno de
        apertura (`US-5.1.2`) sobre el mismo stub."""
...
def _esperar_mensajes_de_cierre(servidor: _FakeSmtpServer, cantidad: int) -> None:
    for _ in range(40):
        if len(servidor.mensajes_de_cierre()) >= cantidad:
            return
        time.sleep(0.05)


# --- Escenario: Cierre manual de actividad restringida a comisiones específicas ---


@given("una actividad vigente restringida a las comisiones A (2 estudiantes) y B (1 estudiante)")
def actividad_vigente_restringida_a_a_2_y_b_1(context, monkeypatch):
...
def verificar_3_emails_de_cierre(context):
    assert context["response"].status_code == 200
    mensajes_cierre = context["servidor_smtp"].mensajes_de_cierre()
    assert len(mensajes_cierre) == 3
    destinatarios = {mensaje.split("To: ")[1].split("\n")[0] for mensaje in mensajes_cierre}
    assert destinatarios == context["emails_esperados"]


@then("cada email contiene el título y la materia")
def verificar_contenido_email_de_cierre(context):
    for mensaje in context["servidor_smtp"].mensajes_de_cierre():
        assert "Parcial 1" in mensaje


# --- Escenario: Cierre manual de actividad sin restricción ---


@given("una actividad vigente sin comisiones_ids, en una materia con las comisiones A y B")
def actividad_vigente_sin_comisiones_ids_con_a_y_b(context, monkeypatch):
    servidor = _FakeSmtpServer()
    context["servidor_smtp"] = servidor
    _apuntar_settings_al_stub(monkeypatch, servidor)
...
def verificar_emails_de_cierre_a_y_b(context):
    assert context["response"].status_code == 200
    mensajes_cierre = context["servidor_smtp"].mensajes_de_cierre()
    destinatarios = {mensaje.split("To: ")[1].split("\n")[0] for mensaje in mensajes_cierre}
    assert destinatarios == context["emails_esperados"]
...
# --- Escenario: Vencimiento natural del período no dispara notificación ---


@given("una actividad vigente cuya fecha_cierre ya pasó, sin cierre manual del Docente")
def actividad_vigente_con_fecha_cierre_pasada(context, monkeypatch):
    servidor = _FakeSmtpServer()
    context["servidor_smtp"] = servidor
    _apuntar_settings_al_stub(monkeypatch, servidor)
...
def corre_verificador_de_vencimientos(context):
    run_async(_correr_verificador_vencimientos())
...
def verificar_no_se_envia_ningun_email_de_cierre(context):
    assert context["servidor_smtp"].mensajes_de_cierre() == []
...
def verificar_segundo_estudiante_recibe_email_de_cierre(context):
...
@then("el cierre responde 200 igual")
def verificar_200_pese_al_fallo_de_envio(context):
    assert context["response"].status_code == 200
