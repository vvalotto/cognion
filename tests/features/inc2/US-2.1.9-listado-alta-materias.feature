...
Feature: Listado y alta de materias (US-2.1.9)
  Como Docente
  Quiero ver el listado de materias
  Para acceder al banco de cada materia (RF-04)

  # Dar de alta una Materia desde la UI pasó a ser exclusivo del Administrador —
  # unificación con Comisión (hallazgo de UAT, Incremento 5-ADJ). El Docente conserva el
  # listado de solo lectura para elegir con qué banco trabajar.
  ...
  Scenario: Alta exitosa de materia
    Given un Administrador autenticado en el listado de materias
  ...
  Scenario: Rechazo por nombre duplicado
    ...
    When un Administrador intenta crear una materia con ese mismo nombre
